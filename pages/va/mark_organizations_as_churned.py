...
def _get_blob_client():
    ...
    container = blob_service.get_container_client(st.secrets["SMARTLEAD_TRIAGE_CONTAINER"])
...
pause_option = st.radio(
    ...
    options=["Pause campaigns immediately", "Schedule pause for a future date", "Don't pause campaigns"],
    index=0,
)
...
confirm = st.checkbox("I understand this will mark the selected organizations as churned")
...
if st.button("Pause organizations", disabled=not confirm or not selected_orgs):
    ...
    if pause_option == "Pause campaigns immediately":
        ...
        st.success(f"Scheduled {count} campaign(s) to pause on {scheduled_date.isoformat()}")
